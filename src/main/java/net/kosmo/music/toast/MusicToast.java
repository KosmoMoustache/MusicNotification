...
import net.kosmo.music.utils.TextHelper;
...
public class MusicToast implements Toast {
    ...
    public Toast.Visibility draw(DrawContext context, ToastManager manager, long startTime) {
        ...
        context.drawText(manager.getClient().textRenderer, TextHelper.getText(this.title), 30, 7, -11534256, false);

        if (ClientMusic.config.TOAST_CONFIG.SHOW_AUTHOR) {
            context.drawText(manager.getClient().textRenderer, TextHelper.getText(this.author), 30, 18, -16777216, false);
        }
        if (ClientMusic.config.TOAST_CONFIG.SHOW_ALBUM_NAME) {
            ClientMusic.drawScrollableText(context, manager.getClient().textRenderer, TextHelper.getText(this.albumName), 30, 30, 29, this.getWidth() - 4, 29 + manager.getClient().textRenderer.fontHeight, Colors.BLACK, false, context.getScaledWindowWidth() - 160 + 30, 0, context.getScaledWindowWidth() - 4, 44);
        }
