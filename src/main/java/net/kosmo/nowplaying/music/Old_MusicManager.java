...
@Deprecated
public class Old_MusicManager {
    private Map<String, Entry> music_list = new HashMap<>();

    public Old_MusicManager(JsonObject json) {
    ...
    @Nullable
    public Entry getByIdentifier(Identifier identifier) {
        for (Entry entry : music_list.values()) {
            if (entry.identifier != null && entry.identifier.equals(identifier)) {
                return entry;
            }
        }
        return null;
